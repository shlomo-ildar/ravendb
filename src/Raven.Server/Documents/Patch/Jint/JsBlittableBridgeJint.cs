...
{
    public class JsBlittableBridgeJint : JsBlittableBridge
    {
        private readonly Engine _engine;

        static JsBlittableBridgeJint()
        {
        }

        public JsBlittableBridgeJint(ManualBlittableJsonDocumentBuilder<UnmanagedWriteBuffer> writer, BlittableJsonDocumentBuilder.UsageMode usageMode, Engine engine)
        ...
        private void WriteInstance(ObjectInstance jsObject, IResultModifier modifier, bool isRoot, bool filterProperties)
        {
            ...
            modifier?.Modify(new JsHandle(jsObject));

            if (jsObject is BlittableObjectInstanceJint blittableJsObject)
        ...
        {
            var writer = _writer;
            var boi = parent as BlittableObjectInstanceJint;
        ...
        private IEnumerable<KeyValuePair<JsValue, PropertyDescriptor>> GetObjectProperties(ObjectWrapper objectWrapper)
        {
            ...
            if (target is IDictionary dictionary)
            {
                ...
                {
                    var jsValue = JsValue.FromObject(_engine, entry.Value);
                ...
                task.IsCompleted == false)
            {
                foreach (var property in type.GetProperties(BindingFlags.Instance | BindingFlags.Public))
                {
                    ...
                    var descriptor = ObjectWrapper.GetPropertyDescriptor(_engine, target, property);
            ...
            {
                var descriptor = ObjectWrapper.GetPropertyDescriptor(_engine, target, field);
        ...
        private unsafe void WriteBlittableInstance(BlittableObjectInstanceJint obj, bool isRoot, bool filterProperties)
        {
            HashSet<string> modifiedProperties = null;
            if (obj.DocumentId != null &&
                _usageMode == BlittableJsonDocumentBuilder.UsageMode.None)
            {
                var metadata = obj.GetOrCreateJint(Constants.Documents.Metadata.Key);
        ...
        public static BlittableJsonReaderObject Translate(JsonOperationContext context, Engine scriptEngine, ObjectInstance objectInstance, 
            ...
            bool isRoot = true)
        {
            ...
            if (objectInstance is BlittableObjectInstanceJint boi && boi.Changed == false && isRoot)
            ...
            using (var writer = new ManualBlittableJsonDocumentBuilder<UnmanagedWriteBuffer>(context))
            {
                ...
                var blittableBridge = new JsBlittableBridgeJint(writer, usageMode, scriptEngine);
                blittableBridge.WriteInstance(objectInstance, modifier, isRoot: isRoot, filterProperties: false);
